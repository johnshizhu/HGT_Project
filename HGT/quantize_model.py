...
from quantize_hgt import quantHGTLayer
...
class quantHGTModel(nn.Module):
    def __init__(self, input_dim, hidden_dim, num_node_types, num_edge_types, num_heads, num_layers, dropout, prev_norm = False, last_norm = False, use_rte = False):
        super(quantHGTModel, self).__init__()
        ...
        for j in range(num_layers - 1):
            self.hgt_layers.append(quantHGTLayer(hidden_dim, hidden_dim, num_node_types, num_edge_types, num_heads, prev_norm, use_rte))
        # Last layer for controlling normalization, etc. for last layer
        self.hgt_layers.append(quantHGTLayer(hidden_dim, hidden_dim, num_node_types, num_edge_types, num_heads, last_norm, use_rte))
